...
本项目为无人拖拉机田头区域全覆盖路径规划提供了一套**完整的商业化解决方案**。经过多轮迭代和优化,最终实现了集成Clothoid曲线、速度规划、曲率约束验证、电子围栏检查、静态障碍物避障等关键特性的生产级路径规划系统。
...
obstacles = [
    # 障碍物1: 水塔 (100m × 100m)
    [(1500, 100), (1600, 100), (1600, 200), (1500, 200)],
...
```python
# 1. 计算主作业区域（扣除田头）
main_work_boundary = field_boundary.buffer(-headland_width)
...
```python
# 1. 创建田头区域
headland_area = field_boundary.difference(inner_boundary)
